...
void TIM3xx::connect(std::string host, int port) {
	if (!connected) {
...
bool TIM3xx::isConnected() {
	return connected;
}

void TIM3xx::startMeas() {
	char buf[100];
	...
	int len = read(sockDesc, buf, 100);
		...
		if (debug) {
			buf[len] = 0;
			std::cout << "startmeasure " << buf << std::endl;
		}
}

void TIM3xx::stopMeas() {
	char buf[100];
...
status_t TIM3xx::queryStatus() {
	...
	int ret;
	sscanf((buf + 10), "%d", &ret);
	//std::cout << "query status " << ret << std::endl;
	return (status_t) ret;
}
...
scanCfg TIM3xx::getScanCfg() const {
	scanCfg cfg;
	char buf[100];
	...
	int len = read(sockDesc, buf, 100);
		...
		if (debug) {
			buf[len] = 0;
			std::cout << "getScanCfg" << buf << std::endl;
		}
...
void TIM3xx::setScanCfg(const scanCfg &cfg) {
	char buf[100];
...
void TIM3xx::setScanDataCfg(const scanDataCfg &cfg) {
	char buf[100];
	...
	int len = read(sockDesc, buf, 100);
	std::cout << "setScanDataCfg " << buf << std::endl;
	buf[len - 1] = 0;
}

void TIM3xx::scanContinous(int start) {
	char buf[100];
	sprintf(buf, "%c%s %d%c", 0x02, "sEN LMDscandata", start, 0x03);
	std::cout << "scanContinous " << buf << std::endl;
	write(sockDesc, buf, strlen(buf));

	int len = read(sockDesc, buf, 100);
	std::cout << "scanContinous returnval " << buf << std::endl;
	if (buf[0] != 0x02){
		printf("invalid packet recieved\n");
		ofLogNotice("ofxSickGrabber") << "invalid packet recieved";
	}
	if (debug) {
		buf[len] = 0;
		printf("%s\n", buf);
...
#define DATA_BUF_LEN 10000
void TIM3xx::getData(scanData& data) {
...
*/
	...
	while(true) {
		...
		tv.tv_sec = 0;
		tv.tv_usec = 50000;
		cout << "select " << sockDesc << endl;
		cout <<"rfds" << &rfds << endl;
		cout << "tv " << &tv << endl;
		cout << "sockDesc" << endl;
		int retval = select(fd_max+1, &rfds, NULL, NULL, &tv); // maybe only once?
...
//			cout << endl;
	...
	cout << "done with Number Data " << endl;
...
void TIM3xx::startDevice() {
	...
	if (buf[0] != 0x02)
		...
		if (debug) {
			buf[len] = 0;
			std::cout << "startDevice " << buf << std::endl;
		}
}
